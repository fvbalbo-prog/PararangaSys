...
export default function AdminCadLanchasScreen() {
  ...
  return (
    <SafeAreaView style={styles.root} edges={['top']} testID="cad-lanchas-screen">
      <View style={styles.header}>
        <Pressable onPress={() => router.back()} hitSlop={16} testID="back-button">
          <Ionicons name="chevron-back" size={26} color={colors.onSurface} />
        </Pressable>
        <View style={{ flex: 1 }}>
          <Text style={styles.title} testID="lanchas-title">Lanchas</Text>
          <Text style={styles.subtitle}>Embarcações por cliente</Text>
        </View>
      </View>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={[styles.card, item.active === false && styles.cardInactive]} testID={`lancha-client-${item.cpf}`}>
      ...
      <AppDialog visible={!!dialog} title={dialog?.title || ''} message={dialog?.message} buttons={dialog?.buttons || []} onRequestClose={closeDialog} testID="lanchas-dialog" />
    </SafeAreaView>
  );
}
